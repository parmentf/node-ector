...
function ConceptNetworkState(conceptNetwork) {
  ...
  var cns = {
    ...
    oldComputeInfluence: function oldComputeInfluence() {
      var _this3 = this;
      ...
      return new Promise(function (resolve, reject) {
        ...
        return promise
        */
        var promise = Promise.all(_this3.cn.node.map(function (node) {
          return _this3.getOldActivationValue(node).then(function (oldActivationValue) {
            return _this3.cn.getNodeFromLinks(node.id).then(function (outgoingLinks) {
              return Promise.all(outgoingLinks.map(function (linkId) {
                return _this3.getLink(linkId).then(function (link) {
    ...
    computeInfluence: async function computeInfluence() {
      ...
      await Promise.all(this.cn.node.map(async function (node) {
        var oldActivationValue = await _this4.getOldActivationValue(node);
        var outgoingLinks = await _this4.cn.getNodeFromLinks(node.id);
        var influencesTo = await Promise.all(outgoingLinks.map(async function (linkId) {
          var link = await _this4.cn.getLink(linkId);
          return {
            value: 0.5 + oldActivationValue * link.coOcc,
            toId: link.toId
          };
    ...
    propagate: async function propagate() {
      ...
      this.cn.node.forEach(function (node, id) {
        ...
        if (!influenceValue[id]) {
          ...
          influence /= Math.log(_this5.normalNumberComingLinks + nbIncomings) / Math.log(_this5.normalNumberComingLinks);
        ...
        _this5.setActivationValue(id, newActivationValue);
      });
    } // propagate
