  ...
  var cns = {
    ...
    computeInfluence: function computeInfluence() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this7.cn.node.forEach(function (node, nodeId, nodes) {
          _this7.getOldActivationValue(node).then(function (ov) {
            var outgoingLinks = _this7.cn.getNodeFromLinks(nodeId);
            outgoingLinks.forEach(function (linkId) {
              var link = _this7.cn.getLink(linkId);
        ...
        resolve({ influenceNb: influenceNb, influenceValue: influenceValue });
      });
    },
